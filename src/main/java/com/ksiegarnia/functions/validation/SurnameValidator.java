...
public class SurnameValidator implements Validator<String> {
    ...
    public void validate(FacesContext context, UIComponent component, String value) throws ValidatorException {
        ...
        if (username == null || username.trim().isEmpty()) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Błąd", "Nazwisko użytkownika jest wymagana."));
        }
        ...
        if (!username.matches(usernamePattern)) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Błąd", "Nazwisko użytkownika może zawierać tylko litery, cyfry, kropki, podkreślenia i myślniki."));
        }

        // Dodatkowe sprawdzenie długości (np. od 5 do 15 znaków)
        if (username.length() < 5 || username.length() > 15) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Błąd", "Nazwisko użytkownika musi mieć od 5 do 15 znaków."));
        }
    }
